...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    private void attemptLogin() {
        ...
        if (TextUtils.isEmpty(password)) {
            mPasswordView.setError("No password");
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                Map jsonMap = new HashMap(2);
                jsonMap.put("email", mEmail);
                jsonMap.put("password", mPassword);
                JSONObject body = new JSONObject();
                JSONObject credentials = new JSONObject();
                credentials.put("email", mEmail);
                credentials.put("password", mPassword);
                body.put("user_login", credentials);
                Webb webb = Webb.create();
                Response response = webb.post(tokenEndpoint)
                        .useCaches(false)
                        .body(body)
                        .asJsonObject();
                ...
                if(response.isSuccess()) {
                    obj = (JSONObject)response.getBody();
                    if (obj.getBoolean("success")) {
                        clientToken = obj.getString("client_token");
                    } else {
                ...
                Log.e("LoginActivty", "Invalid response!");
                return false;
            }
            if(clientToken == null) {
                Log.e("LoginActivity", "Client token string was unassigned");
                return false;
            }
